...
(async () => {
	const env = new NetworkEnv({ serverUrl: process.env.SERVER_URL || 'http://localhost:8080', timeoutMs: 7000 });
	...
	for (let i = 0; i < 5; i++) {
		...
		console.log(`[env-test] step ${i} -> reward=${reward} done=${done} phase=${inf2.phase} stalled=${inf2.stalled}`);
		if (done) break;
	}
