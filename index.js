...
io.on('connection', socket => {
	...
	socket.on('joinRoom', msg => { if (rooms[msg]) { if (rooms[msg].users.length >= (rooms[msg].maxPlayers || 6)) { socket.emit('roomFull'); logEvent({ scope: 'server', action: 'room_join_reject', gameId: msg, playerId: socket.id, payload: { reason: 'full' } }); return; } if (rooms[msg].state) { socket.emit('roomStarted'); logEvent({ scope: 'server', action: 'room_join_reject', gameId: msg, playerId: socket.id, payload: { reason: 'started' } }); return; } socket.join(msg); rooms[msg].users.push(socket.id); socket.emit('roomJoined', rooms[msg]); io.to(msg).emit('update', buildRoomState(msg)); logEvent({ scope: 'server', action: 'room_joined', gameId: msg, playerId: socket.id, payload: { users: rooms[msg].users.length } }); } else { socket.emit('roomNotFound'); logEvent({ scope: 'server', action: 'room_join_reject', gameId: msg, playerId: socket.id, payload: { reason: 'notFound' } }); } });
